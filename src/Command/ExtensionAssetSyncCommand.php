...
#[AsCommand(
    name: 'extensions:assets:sync',
    description: 'Mirror active extension assets and rebuild generated extension asset registries.',
)]
final class ExtensionAssetSyncCommand extends Command
{
    public function __construct(
        private readonly ActiveExtensionAssetProviderInterface $extensionProvider,
        private readonly ExtensionAssetSyncer $assetSyncer,
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $queue = ActionQueue::create('extension asset sync', [
            new ExtensionAssetSyncAction($this->assetSyncer, $extensions),
        ], context: [
        ...
        if ($dryRun) {
            ...
            if ($json) {
                ...
                $io->title('Extension asset sync dry-run');
                if (null !== $extensionProviderError) {
        ...
        if ($json) {
            ...
            $io->success('Extension asset sync completed.');
        } else {
            $io->error('Extension asset sync failed.');
        }
    ...
    private function writeProviderFailure(SymfonyStyle $io, OutputInterface $output, bool $json, Throwable $error): void
    {
        ...
        $io->error('Active extensions could not be loaded; extension asset sync was not started.');
    }
}
