...
#[AsCommand(
    ...
    description: 'Discover extensions, validate them, and synchronize the extension registry.',
)]
final class ExtensionDiscoveryCommand extends Command
{
    public function __construct(
        private readonly ExtensionDiscoveryDispatcher $dispatcher,
        private readonly ExtensionDiscoveryRunner $runner,
    ...
    private function writeChanges(SymfonyStyle $io, array $changes): void
    {
        ...
        $io->table(['Extension', 'Action', 'Status'], array_map(
            static fn (array $change): array => [$change['extension'], $change['action'], $change['status']],
            $changes,
        ));
    }
