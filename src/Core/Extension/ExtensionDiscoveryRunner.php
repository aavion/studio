...
final readonly class ExtensionDiscoveryRunner
{
    public function __construct(
        private ExtensionDiscovery $discovery,
        private ExtensionRegistryHandler $registryHandler,
    ...
    /**
     * @return WorkflowResult<array{candidate_count: int, change_count: int, changes: list<array{extension: string, action: string, status: string}>}>
     */
    ...
    private function candidatesFrom(WorkflowResult $result): array
    {
        ...
        if (is_array($value)) {
            return array_values(array_filter(
                $value,
                static fn (mixed $candidate): bool => $candidate instanceof ExtensionCandidate,
            ));
        }
