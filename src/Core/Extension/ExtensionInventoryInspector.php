...
final readonly class ExtensionInventoryInspector
{
    public function __construct(private FileInventoryScanner $fileInventoryScanner = new FileInventoryScanner())
    {
    }

    public function inspect(string $directory, int $depth): ExtensionInspection
    {
        $inventory = $this->fileInventoryScanner->scan($directory, $depth);
        $inspectableFiles = $inventory->filesWhere(static fn (string $path): bool => ExtensionFilePolicy::isInspectableExtensionPath($path));
