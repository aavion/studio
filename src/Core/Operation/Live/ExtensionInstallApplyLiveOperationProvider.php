...
use App\Core\Extension\Install\ExtensionZipApplyAction;
use App\Core\Extension\Install\ExtensionZipInstaller;
use App\Core\Workflow\WorkflowResult;

final readonly class ExtensionInstallApplyLiveOperationProvider implements LiveOperationQueueProviderInterface
{
    ...
    {
        return LiveOperationQueueFactory::EXTENSION_INSTALL_APPLY;
    }
    ...
    public function create(array $payload = []): WorkflowResult
    {
        ...
        if (!is_string($installId) || '' === trim($installId) || !is_string($extension) || '' === trim($extension)) {
        ...
        return WorkflowResult::success(ActionQueue::create('extension install', [
            new ExtensionZipApplyAction($this->installer, $payload),
        ], context: [
            'operation' => $this->operation(),
