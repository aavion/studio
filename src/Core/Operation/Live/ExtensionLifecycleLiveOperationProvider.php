...
final readonly class ExtensionLifecycleLiveOperationProvider implements LiveOperationQueueProviderInterface
{
    ...
    {
        return LiveOperationQueueFactory::EXTENSION_LIFECYCLE;
    }
    ...
    {
        $extensionName = $payload['extension'] ?? null;
        $action = $payload['action'] ?? null;

        if (!is_string($extensionName) || '' === trim($extensionName) || !is_string($action) || '' === trim($action)) {
        ...
        if (!$resolution->isAvailable()) {
            return WorkflowResult::failed([
                PhpCliUnavailableAction::message('extensions:lifecycle', $resolution->reason(), [
                    'operation' => $this->operation(),
        ...
        return WorkflowResult::success(ActionQueue::create('extension lifecycle', [
