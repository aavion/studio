...
`MaintenanceModeSubscriber` enforces the environment-backed `APP_MAINTENANCE` flag for main requests. When enabled, public requests receive `503 Service Unavailable` unless the authenticated user exposes ACL access level `9`; operational paths such as `/admin`, `/user/login`, localized login paths, and static asset prefixes remain reachable.
