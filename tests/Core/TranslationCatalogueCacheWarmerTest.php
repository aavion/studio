...
final class TranslationCatalogueCacheWarmerTest extends TestCase
{
    ...
    public function testItAggregatesTranslationsAndWritesFreshnessManifest(): void
    {
        ...
        self::assertFileExists($this->root.'/translations/runtime/.manifest.json');

        $manifest = $this->manifest();
        self::assertIsString($manifest['source_hash']);
        self::assertIsString($manifest['generated_hash']);
        self::assertSame([
            'translations/runtime/messages.de.yaml',
            'translations/runtime/messages.en.yaml',
        ], $manifest['targets']);
    }
    ...
    public function testItSkipsWhenSourceAndGeneratedCataloguesAreFresh(): void
    {
        ...
        file_put_contents($this->root.'/translations/runtime/.manifest.json', json_encode($manifest, JSON_THROW_ON_ERROR | JSON_PRETTY_PRINT));
    ...
    public function testItRegeneratesWhenGeneratedCataloguesDrift(): void
    {
        $warmer = $this->warmer();
        $warmer->warmUp($this->root.'/var/cache/test');
        file_put_contents($this->root.'/translations/runtime/messages.en.yaml', "ui:\n  app:\n    name: Stale\n");
    ...
    {
        $manifest = json_decode((string) file_get_contents($this->root.'/translations/runtime/.manifest.json'), true, flags: JSON_THROW_ON_ERROR);
