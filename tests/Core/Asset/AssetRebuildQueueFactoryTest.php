...
final class AssetRebuildQueueFactoryTest extends TestCase
{
    ...
    public function testItBuildsDevelopmentRebuildQueueWithCacheClearAsFinalAction(): void
    {
        ...
        self::assertStringContainsString('assets:install', $actions[1]->label());
        self::assertStringContainsString('importmap:install', $actions[2]->label());
        self::assertStringContainsString('tailwind:build', $actions[3]->label());
        self::assertStringContainsString('cache:clear', $actions[4]->label());
    ...
    public function testItAddsProductionAssetMapCompileAfterRemovingCompiledAssets(): void
    {
        ...
        self::assertSame('remove_path', $actions[4]->type());
        self::assertStringContainsString('public/assets', $actions[4]->label());
        self::assertStringContainsString('asset-map:compile', $actions[5]->label());
    ...
    {
        return new AssetRebuildQueueFactory($this->root, new PackageAssetSyncer($this->root));
    }
}
