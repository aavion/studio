...
const { default: NativeNotificationPreferenceController } = await loadStimulusController('assets/controllers/native_notification_preference_controller.js');
const { default: NativeNotificationsController } = await loadStimulusController('assets/controllers/native_notifications_controller.js');
...
test('native notifications are emitted only when enabled and browser permission is granted', () => {
    installDom();
    ...
    const controller = new NativeNotificationsController();
    controller.enabledValue = true;
    controller.titleValue = 'Fallback title';
    controller.connect();

    document.dispatchEvent(new CustomEvent('ui-alert:shown', {
        detail: { title: 'Alert title', message: 'Alert body' },
    }));

    assert.deepEqual(notifications, [{
        title: 'Alert title',
        options: { body: 'Alert body' },
    }]);
});

test('native notification preference requests permission before submitting', async () => {
    installDom();
    ...
    const submitEvent = event({ submitter });
