...
Active packages may include an optional `package.php` runtime loader. The loader is included only after the package has been validated and activated. It may load further package-owned PHP files below `src/`, return supported contribution DTOs/providers, or return a callable that does the same. Supported direct contribution DTOs currently include static view injections, dynamic view injections, and package setting definitions. Package PHP should use the declared `PACKAGE_NAMESPACE` root or one of its child namespaces. Loader failures are converted into structured lifecycle diagnostics and mark the package `faulty` instead of breaking the request with a raw exception.
