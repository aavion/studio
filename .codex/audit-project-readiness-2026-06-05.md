...
- **Implementation note:** First setup split completed by keeping `SetupPreflightChecker` as the DB-free checklist facade while moving Composer probing, Tailwind smoke checks, process probing, row construction, requirement groups, detail-row aggregation, and PHP CLI failure-key mapping into focused services. `SetupRunner` now delegates runtime commands, database-ready environment scoping, operation-payload message extraction, and run-input policy validation to focused collaborators. A full DI-assembled `SetupPreflightCheck`/`SetupStepProvider` pipeline remains optional if future setup checks or steps need independent registration.
...
- **Implementation note:** `ContentItem` now stays as a small Doctrine aggregate facade while routing/tree/redirect state, localization/variants, ACL rules, metadata, and revision activation/collection state live in focused entity-state traits backed by a shared `ContentItemInput` validator. Column names and public behavior remain stable, avoiding a schema migration while reducing the aggregate file below the context target.
...
- **Implementation note:** First split completed with `NavigationBuilder` as the public facade and focused collaborators for menu storage, access filtering, URL resolution, tree building, active-state marking, and split-menu slicing. A separate ACL value object remains deferred until the shared ACL/reference model from F-028/F-036 exists.
...
- **Implementation note:** Split completed with `SchedulerRunner` as the execution facade, `SchedulerDueTaskSelector` for due/runnability/skipped-task policy, `SchedulerTaskRunRecorder` for persistent task-run execution, `SchedulerFailurePolicy` for failure thresholds, and `SchedulerRunReporter` for run/task result messages and summaries. The scheduler components are now below the 300-line context target.
...
- **Implementation note:** Split `AclGroupImpactService` into a tagged provider aggregator plus domain-owned reference providers for users, account tokens, content items, content schema versions, and site menu items. The current JSON storage stays in place, but query helpers and cleanup rules are no longer centralized in one cross-domain scanner; new ACL-bearing domains now have an explicit provider extension point before Security/Editor adds heavier permissions.
...
- **Implementation note:** Split completed with `SetupInputNormalizer` for shared CLI/web database-driver, database-URL, prefix, boolean, and default-admin-email normalization plus `SetupInputValidator` for shared web error-key validation and CLI input assertions. Web step-scoped filtering and CLI prompts remain transport adapters around the shared validator.
- **Priority:** Before Release.
...
- **Implementation note:** Rechecked after the shared process foundation. `LiveOperationStarter` and `DeferredMessengerDrainProcessStarter` now delegate detached execution to `DetachedProcessStarter`, which owns output/PID marker creation, `CliProcessEnvironment::fromCurrentProcess()`, and Unix/Windows shell detachment. Remaining `exec`/`proc_open` usages are test harness or script-lint helpers, not product subprocess starters.
