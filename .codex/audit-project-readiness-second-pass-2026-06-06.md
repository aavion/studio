...
- Current confidence: second pass completed with all production domains reviewed, all production PHP files at or below `299` lines, and remaining larger lifecycle/security/mail decisions explicitly deferred.

### Domain Progress

| Domain | Scope | Status | Notes |
| --- | --- | --- | --- |
| Backend | `src/Backend` | Reviewed | Admin settings/system-info path, backend view context, route/action naming, and controller adapters reviewed. System-info uses a reduced admin-only report, not raw `phpinfo()` or `$_SERVER`; S2-021 renames internal backend form request attributes to `system`, and S2-027 splits package detail file/link/dependency helpers out of the package detail read-model assembler. |
| Command | `src/Command` | Reviewed | Commands are small, use Symfony-style domain names instead of product-prefixed names, and delegate process-heavy work into services. |
...
| Controller | `src/Controller` | Reviewed | Public, backend, setup, scheduler, operation, account, profile, API-key, invitation, registration, admin user, admin review, and package controllers reviewed as HTTP adapters. S2-009 hardens profile language persistence, S2-028 centralizes repeated token/password helper logic, S2-042 moves admin invitation/token actions behind Security services, S2-043 extracts account-link acceptance mutations, S2-044 moves admin account update/password-reset mutations into Security services, and S2-045 moves authenticated profile/password/closure workflows into focused services. |
...
| Core package | `src/Core/Package` | Reviewed | Package discovery, registry sync, dependency resolution, activation/removal/purge, fault reset, runtime loader/failure handling, package install apply, scheduler cron validation, PHP capability policy, runtime contribution registry, settings, assets, and asset registry contributions reviewed. S2-004 keeps cron parser behavior, S2-006 hardens dynamic callable bypasses, S2-007 records the remaining lifecycle transaction boundary, S2-010 converts package runtime contribution failures to Message-layer diagnostics, S2-026 converts asset contribution invariants to Package Message keys, S2-046 splits activation/removal planning, status storage, filesystem removal, purge, and activation finalization into focused lifecycle services, S2-047 splits scheduler cron inspection into call scanning, import resolution, and argument parsing, and S2-048 splits registry sync finalization plus dependency metadata reading out of large package services. |
| Core observability | `src/Core/Log`, `Statistics`, `Diagnostics` | Reviewed | Visitor/request ID, access metadata sanitization, statistics recorder/aggregator/store, log parsing/filtering/presentation, audit/operation/message logging, and reduced system diagnostics reviewed. S2-016 hardens snapshot temp-file writes, S2-025 renames internal log channels/files to descriptive `message`/`audit`/`access` names under `var/log/{APP_ENV}`, S2-031 validates statistics trace IDs, and S2-034 normalizes statistics-store paths plus deterministic system-info extension output. Public CSS/UI names remain product-facing. |
...
| Debug and Kernel | `src/Debug`, `src/Kernel.php` | Reviewed | Debug collector naming, output safety, and APP_DEBUG gating reviewed. S2-021 renames the internal collector and debug HTML comment to `system`; public Twig helper names now use branding-neutral domain names. |
...
| View | `src/View` | Reviewed | Template runtime fallback, Markdown rendering/embed output, package macro/template paths, system package metadata, Twig helper ownership, dynamic/static view injection registry, response header/output hooks, HTTP error rendering, and dynamic injection failure reporting reviewed. S2-005 removes a hardcoded `en` fallback from the root layout, S2-023 moves Markdown embed accessibility copy to translations, S2-024 converts unsupported template namespace failures to View Message keys, and public Twig helpers now use branding-neutral domain names. |
| Assets/Templates/Translations | `assets`, `templates`, `translations` | Reviewed | Hardcoded language variants, package translation fallback policy, active setup templates, project-owned asset naming, template sizes, source translation catalogues, and asset checks reviewed. S2-022 replaces the package `languages/en` special case with a configured fallback-locale requirement. S2-023 fixes Markdown embed UI copy. S2-040 splits the setup wizard render target into focused partials. S2-041 moves the internal operation-overlay storage key to `system`. Native CSS classes, template IDs, and CSS variables now use branding-neutral `system-*` owner/token names. |
| Documentation | `dev/draft`, `dev/manual`, `docs`, `.codex` | Reviewed | Drafts/manuals/docs rechecked against visitor/session identity, package lifecycle/discovery, package settings/assets, setup PHP CLI resolution, mail deferrals, system/studio naming, and cookie policy. Existing docs describe public service entry points rather than every new internal helper; class map and worklog were updated for changed callables and audit decisions. |
...
- **Recommendation:** Rename internal generated log filenames to descriptive `message`/`audit`/`access` names and move them below `var/log/{APP_ENV}`; use `system_*`/`system.*` only for shared technical namespaces such as service tags and session keys. Leave product-facing Twig helpers, CLI command names, CSS classes, and installation-chosen DB prefixes untouched.
- **Fix applied:** Renamed Monolog channels/files to `message`, `audit`, and `access` under `var/log/{APP_ENV}/`; updated the manual setup-script logger, service tag names to `system.event_hook_provider` and `system.backend_view_provider`; changed the setup wizard session key to `_system_setup_wizard`; updated log source patterns, docs, and tests.
- **Priority:** Now / Final naming gate.
...
- **Finding:** Before the branding-neutral naming migration, most `studio-*` hits in assets/templates were public UI classes, CSS variables, Twig helpers, or product-facing theme API, but `operation_overlay_controller.js` also used `studio.operation...` as an internal `sessionStorage` key for resumable live operations.
...
- **Recommendation:** Use `system.*` for internal storage keys and migrate native CSS/Twig names to branding-neutral `system`/domain namespaces before public release.
...
### S2-043 Public account-link acceptance mixed mutation policy into the registration controller

- **Area:** Public registration/invitation acceptance, account creation/reactivation, group assignment, state markers, audit logging.
- **Finding:** `UserRegistrationController` was 430 lines and still owned the mutation-heavy part of account-link acceptance: resolving duplicate users, reactivating deleted accounts, hashing passwords, applying token role/group state, logging stale token groups, consuming tokens, writing state markers, and auditing acceptance.
- **Evidence:** `src/Controller/UserRegistrationController.php`, `src/Security/AccountTokenLookup.php`, `src/Security/UserGroupMembershipManager.php`, `tests/Controller/UserControllerTest.php`.
- **Impact:** Public form handling and account mutation policy were coupled. Future security hardening around link replay, remember-me issuance, token anomaly logging, or group-policy changes would otherwise need to alter controller-private helper methods.
- **Recommendation:** Keep CSRF/form/password-policy checks in the controller and move the transactional account-link acceptance mutation into a Security service with structured `MessageException` diagnostics.
...
### S2-045 Authenticated user controller still owned account workflow mutations

- **Area:** Authenticated profile, password change, and account closure routes.
- **Finding:** After the account-link and admin-user splits, `UserController` still had 406 lines and mixed HTTP rendering with dynamic profile language option building/application, password-change password-policy mapping, review-token creation/delivery, account closure validation, lifecycle mutation, mail-link/audit side effects, and persistence.
- **Evidence:** `src/Controller/UserController.php`, `src/Security/PasswordPolicyErrorMapper.php`, `src/Security/UserAccountLifecycle.php`, `tests/Controller/UserControllerTest.php`, `tests/Controller/UserProfileControllerTest.php`.
- **Impact:** Authenticated account workflows remained hard to reuse for future API/backend surfaces and made the session/security-sensitive controller larger than the project-rule target.
- **Recommendation:** Keep CSRF/session/form redirects in the controller, but move profile locale option/application, password-change mutation/review-token delivery, and self-service account closure into focused Localization/Security services with structured result objects.
- **Fix applied:** Added `UserProfileLocaleService`, `UserPasswordChangeService`, `UserPasswordChangeResult`, `UserAccountClosureService`, and `UserAccountClosureResult`; reduced `UserController` from 406 to 284 lines; and verified user profile/password/closure flows with focused controller tests.
- **Priority:** Now / Security modularity.

### S2-046 Package activation/removal lifecycle responsibilities were still concentrated

- **Area:** Package activation/deactivation planning, activation finalization, removal planning, filesystem removal, status snapshots/rollback, purge cleanup.
- **Finding:** `PackageActivator` and `PackageRemover` still concentrated multiple lifecycle responsibilities in large files and duplicated package lookup/status snapshot behavior while S2-007's wider lifecycle coordinator remains open for registry/install/runtime-fault boundaries.
- **Evidence:** `src/Core/Package/PackageActivator.php`, `src/Core/Package/PackageRemover.php`, `src/Core/Package/PackageRegistryHandler.php`, `tests/Core/Package/PackageActivatorTest.php`, `tests/Core/Package/PackageLifecycleBoundaryTest.php`.
- **Impact:** Activation/removal behavior was covered but hard to reason about: planning, mutation, rollback, filesystem removal, purge cleanup, and rebuild finalization were mixed into two large services. That made future lifecycle hardening more likely to produce one-path-only fixes.
- **Recommendation:** Split the activation/removal flow into planner, store, finalizer, filesystem-removal, and purge collaborators now, while keeping the broader atomic operation-journal/coordinator decision from S2-007 open for package install, registry synchronization, and runtime fault handling.
- **Fix applied:** Added `PackageActivationPlanner`, `PackageLifecycleStore`, `PackageLifecycleFinalizer`, `PackageRemovalPlanner`, `PackageFilesystemRemover`, and `PackagePurger`; reduced `PackageActivator` from 507 to 157 lines and `PackageRemover` from 439 to 260 lines; and verified activation/removal lifecycle tests plus Symfony container wiring.
- **Priority:** Now / Package lifecycle modularity.

### S2-047 Scheduler cron inspection parser was too concentrated

- **Area:** Package scheduler cron validation for package-authored `SchedulerTaskDefinition` registrations.
- **Finding:** `PackageSchedulerCronInspector` had 494 lines and combined public cron extraction, PHP token scanning, import/alias resolution, call-body extraction, argument splitting, comment stripping, named-argument handling, and literal-string validation in one class.
- **Evidence:** `src/Core/Package/PackageSchedulerCronInspector.php`, `src/Core/Package/PackageSchedulerCronValidator.php`, `tests/Core/Package/PackageValidatorTest.php`.
- **Impact:** The validator behavior was intentionally strict and covered, but the parser internals were hard to review. Future support for additional Scheduler factory helpers or PHP syntax variants would otherwise require changing one large class with multiple parser responsibilities.
- **Recommendation:** Keep the public inspector facade stable and split internals into focused scanner/import/argument collaborators so scheduler-policy validation remains deterministic but easier to extend.
- **Fix applied:** Added `PackageSchedulerDefinitionCallScanner`, `PackageSchedulerDefinitionImportResolver`, and `PackagePhpCallArgumentParser`; reduced `PackageSchedulerCronInspector` from 494 to 47 lines; kept all parser collaborators under 300 lines; and verified package validator tests.
- **Priority:** Now / Package validator modularity.

### S2-048 Registry sync and dependency resolution still held reusable package plumbing

- **Area:** Package registry synchronization, registry asset rebuild fallback, package dependency graph resolution.
- **Finding:** After the activation/removal split, `PackageRegistryHandler` and `PackageDependencyResolver` remained above the target size and still owned reusable lifecycle plumbing: package indexing/managed-path detection, registry sync flush/rebuild/fallback completion, and dependency metadata extraction from package manifests.
- **Evidence:** `src/Core/Package/PackageRegistryHandler.php`, `src/Core/Package/PackageDependencyResolver.php`, `src/Core/Package/PackageLifecycleStore.php`, `tests/Core/Package/PackageRegistryHandlerTest.php`, `tests/Core/Package/PackageActivatorTest.php`.
- **Impact:** Registry synchronization and dependency graph logic were functional, but future lifecycle hardening could still duplicate package lookup, asset rebuild fallback, and dependency metadata parsing outside their natural owners.
- **Recommendation:** Reuse `PackageLifecycleStore` for package registry lookup/path policy, move registry sync flush/rebuild/fallback completion into a finalizer, and move dependency metadata extraction into a reader so the resolver owns graph traversal only.
...
- Debug/view internal naming reviewed. S2-021 moves internal debug collector/comment and backend form request attributes from `studio` to `system`; public Twig helpers now use branding-neutral domain names and native CSS classes use the `system` owner namespace.
...
- View/Twig runtime reviewed. Response header hooks keep sensitive header mutations blocked, output hooks are HTML-only, dynamic injection rendering failures already report through View Message diagnostics, and public Twig helper names are branding-neutral/domain-owned.
- Internal logging/service naming reviewed. S2-025 moves Monolog channel/file names, event/backend view tags, and setup wizard session storage to neutral/system naming after the public Twig helper and CSS namespace migration.
...
- Authenticated user profile/password/closure flows reviewed. S2-045 moves profile locale options/application, password-change review-token creation/delivery, and self-service account closure mutations out of the controller into focused services.
...
- Command names reviewed and migrated away from product-prefixed names. Future command names should prefer Symfony-style domain namespaces where no vendor collision exists.
...
- Final `studio` naming scan reviewed. Remaining `studio-*` and `studio_*` usages are product/brand, historical audit notes, DB-prefix fixtures, or deliberately documented examples; native inspectable CSS/template/helper/CLI/log-path names migrated toward branding-neutral `system`/domain ownership. Refactored system-owned technical identifiers use `system` naming.
...
- Remaining deferred decisions are explicit: S2-007 broader package lifecycle operation journal/coordinator, S2-008 copied session plus copied visitor cookie risk scoring, S2-013 Mail Message/API hardening, S2-018 remember-me persistent-token feature, S2-020 APP_SECRET rotation idempotency edge, and package selector validation/rebranding-package follow-ups after the native branding-neutral naming migration.
...
- Extracted authenticated profile locale, password-change, and self-service account-closure workflows into focused Localization and Security services.
