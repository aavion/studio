...
## Structure
- Each document starts with an H1 title (`# Title`), followed by status metadata (see [Status Tags](#status-tags) for more).
...
## Naming & Files
- User facing documentation goes into `docs/`, for developer documentation use `dev/manual/`.
- Use sub-directories where applicable.
- Developer docs: hyphenated file names (`schema-validation.md`); user docs: hyphenated file names (`content-authoring.md`).
- Feature drafts under `dev/draft/` use prefixes, where applicable (`feat-`, `module-`).
...
## Cross-Referencing
- Link to other docs via relative paths (`[Developer Manual](../dev/manual/README.md)`).
- When referencing sections, use explicit anchors (`[See draft workflow](draft-workflow.md#commit)`).
- Update `dev/WORKLOG.md` when documentation changes require follow-up.

## Screenshots & Media
- Store images under `docs/assets/` (`dev/manual/assets` for developer docs); include captions or context in the surrounding text.
...
- Use templates in `docs/templates/` when creating new guides:
  - `docs/assets/template.md`
  - `dev/manual/assets/template.md`
