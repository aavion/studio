...
- **Decision recorded:** Package-owned CSS should use stable owner/surface/scope naming. System-owned selectors should use `system-*`, `system-frontend-*`, `system-backend-*`, or `system-{provider-scope}-*`; package selectors should use `{package-slug}-*`, `{package-slug}-frontend-*`, `{package-slug}-backend-*`, or `{package-slug}-{provider-scope}-*`. `Studio`/`studio` is product branding, not the default technical selector namespace. Package validation should eventually check selectors and generated assets against the package slug and declared scope.
