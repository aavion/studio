...
- Core hook failures emit `App\Core\Event\PublicHookFailedEvent` as an internal diagnostic signal. Package lifecycle may subscribe later to mark, count, or deactivate the offending package when ownership can be identified.
- Deactivate the offending package when a package-owned listener or handler fails in a way the workflow cannot safely ignore and package ownership can be identified. The deactivation decision belongs to lifecycle code, not to the dispatcher itself.
