...
- Load optional package PHP through `package.php` only for active, valid packages. The loader may require package-owned PHP below `src/`, and hard loader failures should be recorded through lifecycle diagnostics, mark the package `faulty`, and avoid keeping partial runtime contributions from the failed load.
