...
- Use the current public theme hooks for extension work that belongs outside template fallback: `ViewContextEvent`, `ContentRenderContextEvent`, `ResponseHeadersEvent`, and `OutputGeneratedEvent`.
...
- **Decision recorded:** Theme packages should extend rendering through documented context, response-header, and final-output hooks where needed; template namespace discovery and package asset sync do not need separate public collection hooks for now.
