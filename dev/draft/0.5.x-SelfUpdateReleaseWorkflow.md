...
- Keep required application manifest keys aligned with the root `.manifest`:
  ...
  - `APP_CHANNEL`
  - `APP_SOURCE`
- Extend manifests carefully for extension type, compatibility, checksums, signatures, migration requirements, and dependencies.
...
- Check extension compatibility before and after core updates.
- Require backup or backup acknowledgement before destructive updates.
- Validate extension contents before replacing files.
...
- Test extension validation rejects missing required files and unsafe metadata.
- Test update preflight blocks incompatible extensions.
...
- **Decision required:** Decide extension signature/checksum strategy.
