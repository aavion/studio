...
- **Decision recorded:** The setup UI must not touch Doctrine/ORM before the final runner step. Preflight and wizard forms should use filesystem, environment, PHP extension, and process checks only.
- **Decision recorded:** Database configuration is driver-aware. SQLite should favor the generated project-local URL and show a caution for custom URLs; server databases should expose structured connection fields and optional table prefix support through `APP_DATABASE_PREFIX`.
...
- **Decision recorded:** Setup execution should render structured ActionLog progress and route recoverable errors back to the relevant wizard step. Successful setup redirects or offers a start-page action after `APP_SETUP_COMPLETED` is written.
