...
- Run `php bin/console studio:assets:rebuild` when package changes affect templates, CSS, JavaScript, imported assets, or Tailwind class usage.
...
- **Decision recorded:** Web setup preflight should check writable runtime/config paths and dependency/build prerequisites similar to `bin/init`, offer safe automatic repair where setup can run the existing recovery command, and otherwise provide explicit operator instructions.
- **Decision recorded:** The database step should present driver-aware fields. SQLite keeps the project-local default and warns that changing the URL is advanced; MySQL/MariaDB and PostgreSQL expose host, port, database, user, password, and optional table prefix fields.
- **Decision recorded:** `APP_DATABASE_PREFIX` is an optional setup/env key for installations that need table prefixes. Prefix handling should be centralized in the setup/database connection layer so application queries and repositories do not manually concatenate table names.
...
- **Decision recorded:** Web setup execution should use the ActionLog/LiveOperation presentation path where practical, and failure pages should keep the wizard state so the user can jump back to the relevant step, for example database connection settings after a connection error.
