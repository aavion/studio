...
- Run `php bin/console assets:rebuild` when extension changes affect templates, CSS, JavaScript, imported assets, or Tailwind class usage.
...
- Test automation should cover the command order for extension asset rebuilds once lifecycle commands exist.
...
- **Decision recorded:** Provide `bin/init` as a dependency and environment check that verifies Composer packages, importmap assets, Tailwind binary availability, and similar setup prerequisites, and may attempt safe auto-repair such as running Composer install when Composer packages are missing.
...
- **Decision recorded:** `APP_SETUP_COMPLETED` must only be marked after every setup runner step has succeeded. Dry-runs, failed writes, failed migrations, failed seeding, failed extension discovery, or failed asset rebuilds must leave setup unlocked.
...
- **Decision recorded:** Extension asset rebuild automation runs Tailwind before AssetMapper compilation and clears cache last. Watchers are optional development tooling, not the production/admin lifecycle mechanism.
