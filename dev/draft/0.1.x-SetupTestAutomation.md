...
- **Decision recorded:** `APP_DATABASE_PREFIX` is an optional setup/env key for installations that need table prefixes. Prefix handling is centralized through the Doctrine connection wrapper and ORM metadata listener so DBAL helpers, raw DBAL SQL, migrations, setup seeding, Messenger, and repositories share the same known Studio table prefix behavior.
