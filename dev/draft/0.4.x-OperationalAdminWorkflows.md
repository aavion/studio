...
The ActionLog model is the live operation overlay first. It may later feed a structured logger before responses return or jobs complete, using message levels such as `error`, `warning`, `info`, and `debug`. That future logger should be added through an explicit recorder/service boundary; generic operation-message events are deferred until the logger shape exists. Access logs, audit logs, and security logs remain separate layers even when they reuse shared message vocabulary.
...
- Keep ActionLog entries compatible with a later structured logger that can persist `error`, `warning`, `info`, and `debug` events without turning the live overlay into the audit or access log.
