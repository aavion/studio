...
- Support package asset alignment as a first-class operational action that runs `assets:rebuild`: package asset mirror and registry rewrite, translation aggregation, `assets:install`, `importmap:install`, `tailwind:build`, production-only `public/assets` cleanup and AssetMapper compilation, then `cache:clear`.
...
- **Decision recorded:** Package lifecycle changes that affect frontend contributions automatically trigger `assets:rebuild` through the operational action-log workflow.
