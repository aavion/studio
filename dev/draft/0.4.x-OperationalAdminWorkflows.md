...
- Resolve live operation queues through tagged providers so Core, Admin, and later packages/updaters can add operations without turning the runner into a monolithic switch.
...
- Support package asset alignment as a first-class operational action that runs `studio:assets:rebuild`: package asset mirror and registry rewrite, translation aggregation, `assets:install`, `importmap:install`, `tailwind:build`, production-only `public/assets` cleanup and AssetMapper compilation, then `cache:clear`.
...
- **Decision recorded:** Live operations are serialized by a global runner lock in the live operation store. The lock is owner-token protected, touched while the runner reports progress, released in `finally`, and cleaned as stale after the same 3600-second TTL used for transient operation artifacts.
...
- **Decision recorded:** Symfony runtime translation catalogues live below `translations/runtime/messages.{locale}.yaml`; source catalogues stay under `translations/languages/{locale}` so new locales remain discoverable while local conflict files in the translation root are not loaded by Symfony.
...
- **Open:** Define the final provider capability boundary before `1.0.0`. Providers should validate their own payload shape and actor intent, but the core contract should avoid pretend manifest permission flags or overly narrow restrictions that block legitimate package, editor, setup, backup, or updater flows.
