...
- **Decision recorded:** Access, audit, security, and operational logs are separate layers even when they share message keys, severity levels, or UI rendering patterns. The first concrete channels are `studio_message`, `studio_operation`, `studio_audit`, and `studio_access`, each file-based with 30-day retention.
- **Decision recorded:** `studio_operation` receives compact terminal live-operation summaries from the run store. Summaries include operation id/name, status, timing, step count, message count, issue count, and continuation availability, but do not include payloads, polling tokens, or raw runner output.
