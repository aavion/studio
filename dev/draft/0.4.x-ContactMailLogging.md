...
- **Decision recorded:** Separate `studio_message`, `studio_operation`, `studio_audit`, and `studio_access` channels; keep raw request-derived logs separate from anonymized statistics output.
- **Decision recorded:** Access statistics start as an aggregation boundary over the raw `studio_access` files and persist an anonymized latest snapshot through an explicit statistics store. The snapshot exposes counts only, including total requests, approximate unique visitors derived from IP/user-agent combinations, status families, top routes, frequent 404 routes, and countries; it does not expose IP addresses, user-agents, or visitor hashes. The first store is file-backed below `var/statistics/{environment}/access/latest.json` so a later scheduler or database-backed aggregate can replace the producer/store without changing the Admin Logs UI.
