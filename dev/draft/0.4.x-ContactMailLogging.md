...
- Consider a separate admin audit log for high-impact administrator actions, but keep the first implementation flexible enough to use the normal logger with an audit category/channel.
...
- **Decision recorded:** Prefer filesystem-backed structured logs with stable records that can be emitted as or converted to JSONL for UI filtering.
- **Decision recorded:** Separate `access`, `error`, and `security` channels; keep raw request-derived logs separate from anonymized statistics output.
...
- **Decision recorded:** First audit-worthy events should include configuration, protected setting, provider, package lifecycle, schema, import, backup restore, user/ACL, API token, and high-impact maintenance changes.
- **Decision recorded:** Defer a generic operations-message event until after the logger is implemented; start with an explicit recorder/service boundary and keep audit/access/security logs separate from the live ActionLog overlay.
- **Open:** Decide exact log channels, log levels, retention windows, rotation behavior, and whether admin audit uses a separate log or a dedicated category in the normal logger.
