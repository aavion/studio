...
- **Decision recorded:** Access statistics start as an aggregation boundary over the raw `studio_access` files and persist an anonymized latest snapshot through an explicit statistics store. The snapshot exposes counts only, including total requests, status families, top routes, frequent 404 routes, and countries; it does not expose IP addresses. The first store is file-backed below `var/statistics/{environment}/access/latest.json` so a later scheduler or database-backed aggregate can replace the producer/store without changing the Admin Logs UI.
