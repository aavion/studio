...
- Use a separate `studio_audit` channel for high-impact administrator actions, starting with authentication events, backend maintenance/package lifecycle events, and settings saves that log changed keys but not submitted values.
...
- **Decision recorded:** Access statistics start as an aggregation boundary over the raw `studio_access` files. The snapshot exposes counts only, including total requests, status families, top routes, frequent 404 routes, and countries; it does not expose IP addresses.
