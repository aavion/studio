...
- **Implemented baseline:** Added first-pass `ContentItem` and `ContentFieldValue` Doctrine entities plus migration `Version20260527120000`. Entities live under `App\Entity` to keep Symfony's default Doctrine mapping conventions intact, while content-specific enums and routing guards live under `App\Content`.
