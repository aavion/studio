...
| `parent_uid` | Optional parent content reference for hierarchy. Usually a content UID; `system` is reserved as the only virtual parent for internal `/system/...` entities. |
...
- **Decision recorded:** `/system/...` is an internal-only content namespace. It uses the virtual parent marker `system` instead of requiring a real root entity, so system content can be addressed by resolvers without being delivered directly by the public catch-all route.
