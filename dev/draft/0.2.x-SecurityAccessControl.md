...
- **Decision deferred:** The dedicated Security hardening slice should make APP_SECRET rotation handling idempotent per environment/fingerprint pair. If persisting the new fingerprint fails after emergency handling starts, later requests must not repeatedly issue owner recovery links; use a small server-side handling state or attempt journal that can distinguish "handling started" from "handling completed" and audit persistence failures.
