...
- Store API-key lookup hashes and encrypted payloads with `APP_SECRET`-derived material. If the environment-specific `APP_SECRET` fingerprint changes after initial baseline, revoke active API keys and issue password-reset links to active owners through the account-link delivery boundary.
...
- Model ACL groups as contextual memberships with stable identifiers, translatable names, locked preset guardrails, and a single-digit minimum assignable role from 0 to 9. Groups do not determine the user's global role and may be empty.
...
- **Implemented hardening:** `AppSecretRotationGuard` stores an environment-specific APP_SECRET fingerprint after setup. A later fingerprint mismatch revokes active API keys and creates one-hour password-reset links for active owners, then records the new fingerprint and audits the rotation handling.
