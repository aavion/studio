...
- **Decision recorded:** The account-link message-log delivery stub may remain available only as an explicit `APP_DEBUG`/debug aid with strong administrative warning once real mail delivery exists. Production mail should use the real mailer path. Until then, the stub may expose action URLs for local delivery but must not duplicate the plain one-time token as a separate log context value.
