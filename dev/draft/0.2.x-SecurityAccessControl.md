...
- **Decision recorded:** Session hardening should use soft client/visitor binding. Suspicious changes may trigger risk scoring, re-authentication, or administrative review instead of ordinary users being logged out for common network/browser changes.
...
- **Decision deferred:** Soft session/client binding, privacy copy, false-positive handling, and recovery behavior belong to the Security hardening slice. The visitor ID is suitable for statistics and risk signals only; it must not become a hard session hijack defense without that dedicated design.
