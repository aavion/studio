...
- **Decision recorded:** Public content delivery distinguishes access denial from missing or unpublished content for debugging and future security flows. Missing content, unpublished content, and unavailable language/variant contexts return `404`; private or ACL-denied content returns `403`. Once browser authentication exists, anonymous users denied by content ACL should be redirected to the login form with a return target for the original URL; authenticated users who are still denied should receive the configured custom `403` page.
