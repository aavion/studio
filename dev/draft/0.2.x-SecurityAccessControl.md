...
- **Decision recorded:** Visitor identity should use a first-party, server-generated, HMAC-protected, rotatable visitor cookie that respects DNT/opt-out. Avoid machine IDs, advertising IDs, and fingerprinting. IP and user-agent remain soft signals only.
...
- **Decision recorded:** ACL group references should move toward shared access-rule/reference ownership so group impact review, cleanup, package usage, and entity/tree permissions do not rely on ad-hoc JSON scans forever.
...
- **Decision deferred:** Visitor-cookie and soft session/client binding belongs to the Security hardening slice. The current analytics visitor ID remains suitable for statistics and risk signals only; it must not become a hard session hijack defense without a dedicated privacy, false-positive, and recovery design.
