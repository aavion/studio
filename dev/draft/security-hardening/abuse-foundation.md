...
- Admin Logs and `/api/v1/admin/logs/**` read structured message, audit, access, and security-signal sources from database projections. The Symfony application log remains a deliberate file-backed source because it is not projected into the database; it is limited to the existing reverse-tail window and uses stable synthetic file-line IDs for detail links.
