...
- Provider live endpoints, API endpoints, cookies, assets, settings, cache use, external HTTP calls, and validation internals are provider-owned and guarded by existing extension contribution rules. Extension cache artifacts are service-backed, slug-scoped, TTL-bound, and limited to small portable scalar/array payloads.
