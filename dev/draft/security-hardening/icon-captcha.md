...
Implement the first-party IconCaptcha provider as a replaceable extension-owned provider behind the generic captcha contract.
...
- Provider secrets are secret/protected configuration only and must never be stored in extension metadata, public assets, serialized challenge payloads, cache payloads, logs, or diagnostics.
...
- Verify asset licenses/provenance and record the result in branch documentation or extension metadata.
...
- Update extension developer guidance if provider extension layout adds a reusable pattern.
