...
- Existing backend access guard/controller context, Admin controllers, API access guard, live-operation starter/provider boundaries, and settings/extension/scheduler foundations.
- [Security policy defaults](policy-defaults.md).

## Implementation sequence

1. Inventory current Admin surfaces, including settings, users/groups, extension/theme management, scheduler, operations, logs/audit, backups, diagnostics, API management, and future security settings.
...
11. Add extension points for future extension-owned Admin actions only after core action identifiers and collision rules are stable.
...
| `admin.extensions` | Visible | Yes | Extension and theme management area, with mutating lifecycle/install/discovery disabled unless mutable. |
| `admin.backup_restore` | Visible | No | Backup/restore area; restore remains mutating. |
| `admin.extensions.self_update` | Denied | No | System extension self-update transparency row; no current self-update mutation route exists in this slice. |
...
| `admin.settings.extensions` | Visible | Yes | Core extension settings area. |
| `admin.settings.extensions.{extension_slug}` | Mutable | Yes | Active extension-owned settings page, registered dynamically and removed from ACL config during extension purge. |
...
| Extension/theme overview | View installed/available status | View and mutate | Installing, activating, deactivating, updating, purging, and running extension lifecycle actions are Owner-only by default. |
| Extension settings | View and mutate simple non-sensitive extension settings if the extension declares them Admin-safe | View and mutate all extension settings within extension policy | Extension settings that alter routes, permissions, external credentials, data access, or runtime code are Owner-only. |
| Scheduler | View status and run trusted registered tasks when allowed | View and mutate | Enabling scheduler web trigger, GET-token fallback, extension ActionQueue tasks, or destructive tasks is Owner-only. Trusted core tasks run under `admin.scheduler`; extension-provided ActionQueue tasks still require the extension ActionQueue setting and explicit activation. |
...
| Import/apply | View previews where editor/admin permissions allow | View and mutate Owner-sensitive imports | Applying imports that change configuration, extensions, ACLs, users, or system data is Owner-only. Content imports may belong to editor/content ACL policy. |
...
- Extension-owned Admin actions must use extension-scoped identifiers and must not override system action identifiers.
...
- Test Admin can view allowed summaries but cannot mutate Owner-only extension, scheduler, security, backup, update, or protected-secret surfaces.
...
- Test extension-scoped action identifier validation rejects collisions with system actions.
...
- No unbounded or extension-marketplace-style permission editor.
- No extension permission marketplace or manifest permission model.
