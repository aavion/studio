...
- Admins may view normal Admin dashboards, extension/theme overviews, scheduler status, redacted log/audit/security diagnostics, non-secret settings, user review queues, and operational summaries.
...
- Probe-path configuration should use anchored, normalized path patterns with tests that prove common application routes, extension routes, media routes, and editor routes are not accidentally captured.
...
- High-signal probes return generic `400 Bad Request` and must not reveal whether a probed path, file, or extension exists. Probe handling should run before extension loaders and other response-producing request gates, then force a minimal `400 Invalid Request` HTML response for browser probes while leaving the passive response-time signal recorder able to persist the security signal.
...
- Upload and archive handling, including media, extension ZIPs, import bundles, backups, and restore artifacts, should not be treated as suspicious probe traffic by path alone. Failed extension, MIME, size, path traversal, nested archive, and manifest-validation checks should feed passive signals with redacted context.
- Public-facing unsafe form submissions that are not covered by a more specific workflow remain their own `website_form` bucket. This includes future extension-owned public forms such as comments, forum posts, ratings, or similar user-generated content actions.
...
| IconCaptcha provider secret | Secret/protected setting | Yes, protected only | Never stored in extension metadata, public assets, cache payloads, logs, or diagnostics |
