...
- Ban decisions follow the Security policy enforcement order so recovery-login rendering and marked recovery submissions remain reachable, while active bans still run before error pages, API auth success/failure responses, API preflight handling, or rate-limit responses can be produced. Marked recovery submissions are rechecked after authentication and are blocked again unless a trusted user context was established.
