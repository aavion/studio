...
- The recovery login render path `/user/login?bypass=1`, resolved through the shared `RequestPathResolver`, stays reachable despite active Visitor/IP bans. The bypass route keeps its separate strict rate limiting and does not bypass CSRF, credential validation, login-failure accounting, audit logging, or post-login policy re-evaluation.
