...
Simple package settings are registered through `PackageSettingProviderInterface`. A package setting definition provides package name, key, label, default value, value type, optional description/options/metadata, and sort order. The core `PackageSettingRegistry` exposes definitions only for active packages, and `PackageSettings` stores values in `package_setting_entry` instead of global configuration. Admin Settings owns the generic package settings page; packages that need complex workflows should register their own views outside the settings tree.
