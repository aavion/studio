...
- Trusted registered users at or above the configured trusted level, trusted-user-owned API keys, the recovery login render path, and the login submission rendered from that recovery path must bypass active Visitor/IP bans so trusted Owners can recover.
