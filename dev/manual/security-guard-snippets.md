...
- Trusted registered users at or above the configured trusted level, trusted-user-owned API keys, and the recovery login render path must bypass active Visitor/IP bans so trusted Owners can recover. The trusted level defaults to `MANAGER` and is bounded to valid registered-user levels `USER..OWNER`. Login submissions carrying the recovery marker rendered from that recovery path may reach authentication, but active bans are rechecked after authentication unless the login establishes a trusted user context.
