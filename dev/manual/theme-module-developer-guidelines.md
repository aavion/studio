...
- Extension-owned domain data should use the `database` contribution contract. Free-form Doctrine migration classes and arbitrary SQL are not part of the extension contract; core generates physical table names from `{database-prefix}{extension-slug_}{local-table}` and only purges tables under that prefix. Extension tables may use primary keys, regular indexes, unique indexes, and foreign keys to other tables owned by the same extension. Core-owned users, ACL groups, content items, and schemas should be referenced through documented stable lookup/reference contracts once those are available, not through extension-created foreign keys to core tables.
