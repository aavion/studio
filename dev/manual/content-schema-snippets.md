...
Error-page handling uses a layered fallback: when an error such as `404`, `403`, `429`, `451`, or maintenance-mode `503` occurs, `HttpErrorRenderer` first looks for a matching internal content entity such as `/system/error-pages/404` or `/system/error-pages/503`; if none exists or it cannot be rendered, it falls back to `@frontend/error-pages/{status}.html.twig`, then `@frontend/error-pages/default.html.twig`. Direct browser requests to `/system/**` remain blocked by the route guard, but internal rendering may resolve those entities.
...
Content slugs use the centralized content-slug rules: lowercase letters, digits, and single hyphen-separated segments, with no leading or trailing hyphen and a 60-character validation limit. Unlike technical owner slugs, content slugs may start with a digit so natural public paths such as `/2026-report` or `/404` remain possible.

Localized public routes are controlled by `localization.route_prefixes_enabled`. When enabled, the best matching browser language from `Accept-Language` is used for unprefixed redirects when available; otherwise `localization.default_language` is used. For example, `/` redirects to `/de` and `/about` redirects to `/de/about` when `de` is selected. Available prefixes are discovered from generated `translations/runtime/{APP_ENV}/messages.*.yaml` catalogues with `translations/languages/*` as a bootstrap fallback, and active language prefixes are reserved content slugs while localized routing is enabled.
...
Content-domain validation should not throw free-form user-facing sentences. Use a structured `Message` shape with a domain-owned machine-readable message code, a translatable message key, parameters for translators, and optional context for logs or diagnostics.
