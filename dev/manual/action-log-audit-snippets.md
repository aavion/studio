...
Raw access logging and access statistics are separate product surfaces. `studio_access` keeps operational request traces for security and diagnostics, including IP address, proxy hints, user-agent, request id, visitor id, requested path, resolved route, status, duration, content metadata, and GeoIP placeholders. Existing `X-Request-ID` and `X-Correlation-ID` values may be reused when they are already short, safe correlation tokens; otherwise the system generates a fresh request id. Known token-bearing query values, request path segments, and referrer path segments are redacted before logs, trace data, or statistics rows are written. The Monolog rotating handler keeps at most 30 daily files and should remain enabled because future rate-limit and suspicious-traffic features depend on this short-lived operational trail.
