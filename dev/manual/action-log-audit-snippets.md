...
Access logs, audit logs, security logs, and operational action logs may share message levels or rendering helpers, but they should remain separate storage and retention concerns. The first built-in file channels are `system_message`, `system_audit`, and `system_access`, each configured as file-based Monolog channels with 30-day retention.
