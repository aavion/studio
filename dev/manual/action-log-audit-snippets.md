...
Statistics recording, display, and aggregation can be disabled independently from raw access logging through Statistics settings. When disabled, the database recorder skips new rows and Admin Statistics reports the disabled state; raw `access` logging continues for operational security. The first policy setting also respects `DNT: 1` by default for statistics recording only; when DNT is not respected, the request flag is stored and shown in aggregate output. Granular statistic events older than three months are deleted during recording. Long-term compaction beyond that cutoff is intentionally deferred until the final statistic dimensions are known; premature per-field compaction would add complexity before the reporting surface is stable.

Aggregation, recording, and snapshot-store failures should be reported through the message layer so they are visible in `message` without blocking the user request.
