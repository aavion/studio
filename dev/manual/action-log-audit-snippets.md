...
Until the mailer slice exists, account setup and recovery links are delivered through `MessageLogAccountLinkDelivery`. This intentionally writes the generated account link to the message log so invitation, registration, password-reset, and password-change review flows are locally testable. Delivery records include stable `mail_flow_key` values such as `account.invitation.link`, `account.registration.link`, `account.password_reset.link`, `account.registration.approval_requested`, `account.registration.approved`, `account.registration.rejected`, `account.registration.existing_account`, `account.password.changed`, `account.password_change.disputed`, and `account.password_change.reactivated` so the future mailer can map them to localized templates. Existing-account registration notices include the username in context for the mailer while the public UI keeps the same success response as a new registration. Password-change dispute notifications include the username and user UID for administrator review after the review link temporarily locks the account; reactivation notifications tell the user to recover access through password reset after the admin assigns a random password. Replace that delivery implementation with real mail delivery before production use, or keep it only behind an explicit development/debug gate.
