...
Until the mailer slice exists, account setup and recovery links are delivered through `MessageLogAccountLinkDelivery`. This intentionally writes the generated account link to the message log so invitation, registration, and password-reset flows are locally testable. Delivery records include stable `mail_flow_key` values such as `account.invitation.link`, `account.registration.link`, `account.password_reset.link`, `account.registration.approval_requested`, `account.registration.approved`, `account.registration.rejected`, and `account.registration.existing_account` so the future mailer can map them to localized templates. Existing-account registration notices include the username in context for the mailer while the public UI keeps the same success response as a new registration. Replace that delivery implementation with real mail delivery before production use, or keep it only behind an explicit development/debug gate.
