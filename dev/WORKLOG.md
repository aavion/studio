...
### 2026-05-26
- Refined the `user/*` foundation with a DB-backed user-flow config reader, config-gated registration, a login-page registration link only when registration is enabled, a high-sort-order system login/profile menu that can be disabled, authenticated profile children for API keys/invitations/studio/admin/logout, and an API-key page that lists persisted scoped keys for the current user.
...
- Added the first native login/security bridge: `/user/login` now renders the frontend login form, Symfony form login uses `UserAccount` credentials with CSRF, unauthorized backend/security flows return `401` through the shared HTTP error renderer, anonymous `401` responses render the login form, and authenticated users without sufficient ACL level receive the resolved `401` error page instead of backend-local denial markup.
