...
### 2026-06-06
- Continued the second project-readiness audit pass through Content/Scheduler/Security/Setup follow-ups, reported broken content schema custom Twig through the Message layer before fallback, completed the Scheduler domain review notes, removed separate clear-token context logging from the account-link message-log delivery stub while keeping action-url based local delivery until real mailer delivery exists, and split CLI setup database input resolution out of the top-level CLI input factory.
