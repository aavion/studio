...
### 2026-06-07
- Addressed API review hardening findings: public content revision navigation now exposes only active published revision metadata, normal user/admin detail paths reject retained deleted accounts, ACL group deletion no longer requires a JSON content type for empty confirm/review requests, and admin package management routes use canonical package slugs while returning package names as data.
