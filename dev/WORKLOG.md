...
- Added the documented GET-only plural recovery-login render alias and RequestPathResolver coverage so localized `/users/login?bypass=1` recovery renders can bypass active source bans without adding a POST/login-check surface.
