...
- Added operational access metadata for later rate-limit and suspicious-traffic work: request id, duration, surface, host/referrer/language hints, content metadata, and response size now flow into the short-lived access log, while statistics keeps only anonymized or coarse aggregate-safe fields.
...
- Enriched HTTP-backed audit events with request id, visitor id, requested path, and resolved route at the logger boundary so later security automation can count visitor-scoped actions such as failed logins or comment submissions without scraping raw access logs.
...
- Added durable live-operation summaries through the message channel: terminal runs log compact operation id/name, status, timing, entry/message/issue counts, and continuation availability while omitting payloads, polling tokens, and raw runner output.
