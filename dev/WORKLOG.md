...
### 2026-05-31
- Addressed follow-up review hardening and performance cleanup for user-management settings and ACL cleanup: made the ACL role migration drop the legacy access-level index portably on MySQL/MariaDB, validated notification email settings before persistence, allowed clearing the optional default ACL group, warned when deleting the sole published view group would make content public, and narrowed ACL group impact scans to targeted candidates.
