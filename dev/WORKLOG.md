...
### 2026-05-31
- Started `feat-setup-wizard`: split setup into a dedicated DB-free wizard controller with preserved step state, auto-healing preflight plus public-webroot validation, driver-aware database fields with centralized `APP_DATABASE_PREFIX` handling, modular initial settings, first OWNER account input, review/result screens, live-language switching, and per-step shared runner/LiveOperation execution; focused setup lint/container/PHPUnit checks are green.
