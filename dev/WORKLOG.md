...
### 2026-05-28
- Addressed PR review hardening for user management: registration mode references are unified, security-review links require explicit POST confirmation before account locking, public registration honors the configured default ACL group, and self-service closure cannot remove the last active admin.
- Addressed additional account-flow review hardening: mail-bound action links now use the shared absolute URI generator based on configured `site.url`, link-generation failures reach public/admin UI as generic delivery errors without logging tokens, email addresses are normalized centrally to lowercase with case-insensitive account lookup for legacy mixed-case rows, and ACL group identifiers require lowercase snake_case with at least 3 characters everywhere they are accepted.
- Polished self-service account closure: a dedicated confirmation page now shows the configured deleted-user retention window, the `account.closed` mail flow receives `retention_days`, and successful closure logs the user out before redirecting to the homepage.
- Tightened the follow-up review edges: user settings now reject invalid default registration groups, ACL group updates/deletes preserve the configured default group at level 1+, and the last-admin self-closure guard reaches the profile UI as a translated flash message.
- Added a retained deleted-users admin view linked below the account list, hid deleted accounts from the main user list, added configurable deleted-user retention, deleted-account activate/deactivate actions with group-floor repair confirmation and account-restored mail notification, and introduced a reusable cleanup service that permanently removes deleted accounts older than the retention window for later scheduler reuse.
