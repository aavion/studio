...
### 2026-05-31
- Started `feat-setup-wizard`: split setup into a dedicated DB-free wizard controller with preserved step state, field-level validation, DB connection testing, auto-healing preflight plus public-webroot validation, driver-aware database fields with default `studio_` `APP_DATABASE_PREFIX` handling, modular initial settings, first OWNER account input with shared password-policy enforcement and a setup strength meter, review/result screens, live-language switching, per-step shared runner/LiveOperation execution, and a consolidated current baseline migration; focused setup/security/controller PHPUnit checks are green.
