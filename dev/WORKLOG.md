...
- [ ] Migrate inspectable/user-facing native CSS classes, template identifiers, Twig helper names, browser-storage keys, and package-facing generated asset names from hardcoded `studio` branding toward the documented `system` owner and package-slug namespace convention; keep runtime branding sourced from `.manifest`, branding assets, and branding tokens.
- [ ] Review native CLI command namespaces before `1.0.0` and prefer Symfony-style domain commands such as `assets:*`, `packages:*`, and `scheduler:*` where they do not collide with vendor commands; retain product-prefixed aliases only when deliberately useful.
- [ ] Move built-in Monolog file paths toward `var/log/{APP_ENV}/{message|audit|access}-{rotation_date}.log` and keep log filenames descriptive without product or system owner prefixes.
- [ ] Review existing `system` and `studio` identifiers one more time after the naming migration and keep `system` only where it protects a shared namespace; prefer plain domain names such as `visitor_id` where ownership is already clear.
