...
- [ ] Extension storage/upload follow-up: add durable extension-owned storage plus safe upload import helpers that let API/live handlers accept user uploads without `$_FILES`, direct filesystem writes, or high-risk executable/MIME payloads. Prefer blacklist-based high-risk type rejection with size/path/quota guards over a narrow media whitelist.
