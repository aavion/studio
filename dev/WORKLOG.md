...
- Captured IconCaptcha readiness boundary: private assets, GD, and extension-owned live endpoints are available for future challenge rendering, but provider-owned challenge state still needs a documented extension cache facade/context API before implementing the real provider.
