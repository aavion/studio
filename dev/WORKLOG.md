...
### 2026-05-28
- Addressed PR review hardening for user management in focused follow-up commits: unified registration settings, explicit security-review POST confirmation, actor-checked/default ACL group handling in registration, reissue, and approval flows, last-admin/self-closure guards, shared absolute URL generation with generic delivery errors, lowercase email normalization, DB-free setup username validation, stricter ACL identifiers, login password-reset discovery, password-change delivery preflight, stale dispute-token cleanup, and ACL group deletion warnings for content that may become public.
