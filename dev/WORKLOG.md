...
- Addressed first-round review findings by enforcing package live endpoint minimum access levels before handler dispatch, folding `ui_alert_inbox` into the pre-`1.0.0` baseline migration, rejecting duplicate cookie-consent definitions, expiring optional cookies when consent is withdrawn, and switching cookie-consent CSRF to a stateless HMAC token so hidden public consent UI does not create anonymous sessions.
...
- Hardened Mercure local tooling with Bolt storage at `var/mercure/updates.db`, self-healing `mercure:health` startup/probe retries, `mercure:stop` debugging support, OS-aware process termination, and a best-effort macOS quarantine release for downloaded Darwin binaries.
...
- Tightened the Mercure public endpoint probe for UI-alert push delivery to require an anonymous `text/event-stream` subscription, and updated the local hub start environment to inject the `anonymous` Mercure directive so public HMAC-bound alert topics can be consumed without subscriber cookies.
...
- Relaxed read-only Mercure hub reachability diagnostics to accept `2xx`, `400`, and `401` responses so authenticated or missing-topic probe responses do not create false negatives after fresh local hub rebuilds; publish health now requires a successful authenticated POST, and public subscribe checks remain tied to a real anonymous `text/event-stream` response.
...
- Addressed second-round review findings by making cookie consent rejection server-visible without JavaScript, keeping profile pages on cached Mercure availability only, preserving native-notification preferences while the toggle is hidden, moving local Mercure JWT secrets out of command arguments, requiring authenticated publish POST success for Mercure health, scheduling expired UI-alert inbox cleanup, routing live-poll invalid-response text through existing translated operation labels, and scoping notification-center storage by user/session/surface.
- Switched local Mercure tooling from deprecated `mercure-legacy_{OS}_{ARCH}` release assets to the Caddy-based `mercure_{OS}_{ARCH}` binaries, invalidated stale legacy installs with an asset marker, changed `mercure:start` to run the hub via the release-provided Caddyfile plus a protected `var/mercure/mercure.env` file for `SERVER_NAME` and JWT secrets, and kept anonymous/CORS/Bolt transport directives as non-secret process environment.
- Added a sufficiently long local-development `MERCURE_JWT_SECRET` default and made setup write an installation-specific derived Mercure JWT secret so fresh clones and completed setup environments can satisfy `lcobucci/jwt` HMAC key-length requirements.
...
- Verification: `php -l src/Core/Mercure/MercureRuntime.php src/Command/MercureCheckCommand.php src/Command/MercureHealthCommand.php tests/Core/Mercure/MercureRuntimeTest.php`; `php bin/phpunit tests/Core/Mercure/MercureRuntimeTest.php`; focused `bin/lint` for the touched Mercure files, class map, and worklog; repeated `php bin/phpunit tests/Core/Mercure/MercureRuntimeTest.php` after adding MockHttpClient coverage for GET `400`/`401` publish reachability; confirmed `public/assets` is absent.
- Verification: `php -l src/Controller/LiveEndpointController.php tests/Controller/LiveEndpointControllerTest.php src/Privacy/Cookie/CookieConsentRegistry.php src/Privacy/Cookie/CookieConsentManager.php src/Privacy/Cookie/CookieConsentTwigExtension.php src/Controller/CookieConsentController.php migrations/Version20260531000000.php tests/Privacy/Cookie/CookieConsentManagerTest.php tests/Operations/SqliteMigrationTest.php`; `php bin/phpunit tests/Controller/LiveEndpointControllerTest.php tests/Privacy/Cookie/CookieConsentManagerTest.php tests/Operations/SqliteMigrationTest.php --display-phpunit-notices`; `php bin/console lint:twig templates/components/CookieConsent.html.twig`; `php bin/console lint:container`.
