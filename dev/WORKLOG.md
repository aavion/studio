...
- Post-checked feat-user-management review readiness: converted invitation acceptance edge failures to structured messages, preserved parameterized frontend error rendering, and added Symfony firewall access-control rules for admin, editor, and authenticated user surfaces.
- Tightened account-flow edge behavior: public registration remains enumeration-safe, token-only onboarding exposes useful username conflicts, wrong token types now 404 on GET workflows, existing-account invites upgrade/add access without downgrade, deleted-account links preserve surviving role/groups, and profile email changes validate uniqueness through structured UI errors.
