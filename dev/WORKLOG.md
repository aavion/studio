...
- Fixed Cloud Review extension contract findings by registering extension namespaces during activation-only contribution reads, removing the duplicate global captcha enable switch in favor of active provider availability, preventing cross-extension `require_vendor()` package success without allowing duplicate package instances, and deferring additive existing-table updates until missing table creation succeeds. Rejected verified-only ban-recovery captcha because optional captcha providers must not lock owners out when no provider is active.
