...
### 2026-06-01
- Started test-suite performance audit: created a clean audit branch, removed local duplicate test artefacts, measured the full PHPUnit suite at 802 tests / 4960 assertions, and narrowed PHPUnit discovery to `*Test.php` files so editor/cloud duplicate PHP files cannot be collected as accidental tests; current hotspots are backend setup/controller integration tests and one ZIP installer overwrite flow.
