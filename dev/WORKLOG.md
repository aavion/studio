...
### 2026-06-07
- Hardened API review findings around response preservation and package-owned route patterns: endpoint access, endpoint permission, maintenance, and read-only subscribers now preserve earlier API responses such as disabled/setup/maintenance `503`, anonymous `401` challenges, and media-type errors; package API contributions must keep custom path patterns anchored below their owned package subtree; API-key authentication now ignores unrelated non-Bearer authorization schemes on public reads while still rejecting malformed Bearer credentials; content item collection reads now honor capped pagination, schema, parent, and sort query parameters after ACL filtering; and the published-content endpoint no longer advertises or returns draft/deleted status lists before an editor-visible read path exists.
...
- Added an endpoint permission policy and admin permissions matrix: endpoint definitions now reject public mutating operations, OpenAPI and navigation expose neutral `x-access` metadata, `/api/v1/admin/permissions` lists required role, access level, API-key capability, and domain-ACL handoff for every endpoint, and an early subscriber enforces derived minimum access from stable API tag scopes before handlers run.
...
- Split access-statistics snapshots into `page_requests` and `api_requests` while keeping total request counts and existing surface lists, so `/api/**` traffic remains visible without being mistaken for normal page traffic; confirmed external correlation IDs are already captured separately from internal request IDs.
...
- Localized API runtime feedback through the Message layer while keeping OpenAPI and technical response contracts in English; request locale resolution now follows `?language=`/URL, session, user preference, `Accept-Language`, and default language, content API reads fall back to the resolved request locale when no explicit language is passed, and content API metadata messages include translated text.
