...
- Closed the adjacent local-review multi-table DDL rollback finding by preflighting existing extension table updates and rejecting batches that would update more than one existing table before any ALTER runs.
- Closed a local-review database DDL atomicity finding by rejecting existing extension table updates that DBAL expands to multiple DDL statements before any ALTER runs, keeping additive table sync from escaping partial state on non-transactional DDL platforms.
- Removed extension-local PHP Composer dependency support for early releases by blocking root `vendor/`, `composer.json`, and `composer.lock`, deleting the `require_vendor()` runtime facade and Composer artifact validator, and documenting the decision to avoid unsafe package-version conflicts inside one PHP runtime.
- Fixed Cloud Review extension contract findings by registering extension namespaces during activation-only contribution reads, removing the duplicate global captcha enable switch in favor of active provider availability, and deferring additive existing-table updates until missing table creation succeeds. Rejected verified-only ban-recovery captcha because optional captcha providers must not lock owners out when no provider is active.
- Fixed a local-review scheduler redaction gap by treating scheduler run `message`, `exception_message`, and `previous_message` context fields as sensitive before run history/API exposure, with callable failure coverage for provider exception text.
- Documented runtime extension faulting as intentional health enforcement rather than an Admin read mutation: any main request may fault broken active extension code through the central loader, while read-model surfaces must not add separate Admin-only mutations beyond that shared boundary.
- Closed follow-up live-operation leakage paths by redacting sensitive message parameters such as `%message%`, diagnostic `message`/`reason` context, and direct `LiveOperationHttpResponder` / continuation `WorkflowResult` JSON responses through the shared presentation redactor.
- Fixed local-review findings in live-operation presentation and cleanup by projecting terminal polling results and admin-list issues through redacted UI payloads, preserving diagnostic context only for warnings/errors, and reclaiming expired corrupted run state files with matching runner output/PID files.
