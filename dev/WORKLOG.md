...
- Changed the native captcha provider fallback to invisible hidden metadata only, added stable form ids for provider JS targeting, passed dynamic form ids into captcha field rendering, and covered that client-supplied fallback/skip payloads cannot bypass an active provider.
