...
- Localized API runtime feedback through the Message layer while keeping OpenAPI and technical response contracts in English; `?language=` now acts as a global request-locale override, content API reads fall back to the resolved request/user/default locale when no explicit language is passed, and content API metadata messages include translated text.
