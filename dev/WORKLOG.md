...
### 2026-05-31
- Started `feat-setup-wizard`: split setup into a dedicated DB-free wizard controller with preserved step state, field-level validation, DB connection testing, auto-healing preflight plus public-webroot validation, driver-aware database fields with default `studio` prefix input and stored `APP_DATABASE_PREFIX` handling, modular initial settings, first OWNER account input with shared password-policy enforcement and a reusable password-policy checklist/match meter for setup, invitation, reset, and profile password forms, review/result screens, live-language switching, per-step shared runner/LiveOperation execution, and a consolidated current baseline migration; focused setup/security/controller PHPUnit checks are green.
- Refined setup wizard UX and runner safety: live language changes stay on the first step, preflight checks are folded into the language step with ready checks collapsed, CLI subprocess availability, setup command execution, operation process probes, and PHP linting use Symfony Process, table-prefix display/storage is deterministic, route-prefix localization can be seeded, and live-operation JSON errors are handled defensively.
- Extended setup hardening with a shared password-policy match check, 12-character minimum for custom setup hash salts, and prefixed-database seeding under the same temporary environment used for migrations.
...
- Allowed `/api/live/*` during pre-completion setup so JSON polling is not redirected, and added setup rollback cleanup for generated env files plus known prefixed app tables on SQLite, MySQL/MariaDB, and PostgreSQL when setup fails.
