...
### 2026-06-01
- Started `feat-scheduler`: aligned the Scheduler draft with `/cron/run`, API-key triggering, cron expressions, Symfony Scheduler/Messenger integration, task/run persistence, package task policy, admin UI expectations, and failure/logging behavior; added `symfony/scheduler` plus cron-expression support as the implementation base.
