...
| N/A | `App\Core\Filesystem\PathGuard` | Shared relative-path normalizer and traversal guard for filesystem operations scoped to a root. | `dev/draft/0.1.x-CoreArchitecture.md` | `tests/Core/Filesystem/PathGuardTest.php`, `tests/Core/Package/PackageValidatorTest.php` |
...
| N/A | `App\Core\Operation\OperationExecutor` | Bridges dry-run planning and action execution with ActionLog output, result aggregation, and exception mapping. | `dev/draft/0.1.x-CoreArchitecture.md` | `tests/Core/Operation/OperationExecutorTest.php` |
